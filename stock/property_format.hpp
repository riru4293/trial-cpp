...
namespace machine {
     ...
     *         - `machine::PropertyFormat spec` <---[this]
     */
    ...
    public:
        ...
        *
        * @note ja: 下位2ビットを @ref PropertyFormat::Kind に変換します。
    ...
    public:
        ...
        ~PropertyFormat() noexcept = default;                   //!< Destructor (default).
        PropertyFormat(const PropertyFormat &) noexcept = delete; //!< Copy constructor (deleted).
        PropertyFormat(PropertyFormat &&) noexcept = default;     //!< Move constructor (default).
    ...
    public:
        ...
        PropertyFormat &operator=( PropertyFormat const & ) noexcept = delete;          //!< Copy operator (deleted).
        PropertyFormat &operator=( PropertyFormat && ) noexcept = default;              //!< Move operator (default).
        bool constexpr operator==( PropertyFormat const & ) const noexcept = delete;  //!< Equality operator (deleted).
        auto constexpr operator<=>( PropertyFormat const & ) const noexcept = delete; //!< Three-way comparison operator (deleted).
    ...
    /** @brief Stream output operator for `PropertyFormat::Kind`. */
    /**
     * @details
     * Outputs the string representation of the `PropertyFormat::Kind` instance
     * to the provided output stream.
     *
     * @see PropertyFormat::name_of() for the format of the output.
     *
     * @param os The output stream to write to.
     * @param v The `PropertyFormat::Kind` instance to output.
     *
     * @return Reference to the output stream after writing.
     */
    std::ostream &operator<<( std::ostream &os, PropertyFormat::Kind const &v ) noexcept;

    /** @brief Alias of the @ref PropertyFormat::Kind */
    using PropKind = PropertyFormat::Kind;

    /* ^\__________________________________________ */
    /* Static assertions.                           */
    static_assert( sizeof(machine::PropertyFormat) == 20, "Unexpected PropertyFormat size" );
    static_assert( alignof(machine::PropertyFormat) == 1, "Unexpected PropertyFormat alignment" );

} // namespace machine

namespace std {

    /** @brief Formatter specialization for `machine::PropertyFormat::Kind`. */
    /**
     * @details
     ...
     */
    template <>
    struct formatter<machine::PropertyFormat::Kind>
    {
        ...
        /** @brief Format `machine::PropertyFormat::Kind` value. */
        template <typename FormatContext>
        auto format( machine::PropertyFormat::Kind const &v, FormatContext &ctx ) const
        {
            return std::format_to( ctx.out(), "{}({})",
                machine::PropertyFormat::name_of( v ), static_cast<int>( v ) );
        }
    };
